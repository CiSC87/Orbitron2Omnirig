...
def adjust_freq(sat_data):
    ...
    if abs(exact_dl-FREQ_CURR_DL)>FREQ_MAX_DELTA:
...
def display_data(sat_data, rig_data):
    os.system('cls')
    try:
        timestamp_local = datetime.datetime.strptime(sat_data['timestamp_local'],'%Y%m%d%H%M%S')
        timestamp_utc = datetime.datetime.strptime(sat_data['timestamp_utc'],'%Y%m%d%H%M%S')
        ...
        print("Obs. PoV sat. Elevation  :",int(float(sat_data['sat_elevation'])),"deg",end='')
        if float(sat_data['sat_elevation'])>0:
            print(" (above the horizon, visible)")
        else:
            print(colored(" (below the horizon, not visible)","red"))
        print("Obs. PoV sat. Azimuth    :",int(float(sat_data['sat_azimuth'])),"deg",end=' ')
        print("("+deg_to_compass(float(sat_data['sat_azimuth']))+")")
        print("Obs. PoV sat. Distance   :",int(float(sat_data['sat_distance'])),"km")
        print("Obs. PoV sat. Velocity   :",int(float(sat_data['sat_velocity'])*60*60),"km/h",end='')
        ...
        try:
            ...
            while True:
                try:
                    ...
                    print("DDE conversation error: ", e, " - Retrying in 1 sec...")
                    time.sleep(1)
            # Handle data loop
            while True:
